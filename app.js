...
const statusEl=document.getElementById("status"),debugEl=document.getElementById("debug"),shareToolsEl=document.getElementById("shareTools"),listenerStatEl=document.getElementById("listenerStat"),connectedToolsEl=document.getElementById("connectedTools"),listenerCountEl=document.getElementById("listenerCount"),muteBtn=document.getElementById("muteBtn"),leaveBtn=document.getElementById("leaveBtn");
...
function showShareQr(room,role){const url=buildJoinUrl(room);document.getElementById("joinUrlText").textContent=url;document.getElementById("qrImage").src="https://api.qrserver.com/v1/create-qr-code/?size=180x180&data="+encodeURIComponent(url);document.getElementById("qrTitle").textContent=role==="guide"?"遊客掃碼加入":"分享給同行者掃碼加入";shareToolsEl.classList.remove("hidden")}
...
function randomIdentity(role){const prefix=role==="guide"?"Guide":"Listener";return `${prefix}-${Math.floor(100000+Math.random()*900000)}`}
...
const identity=randomIdentity(currentRole);const room=document.getElementById("room").value.trim();if(!/^[0-9]{4}$/.test(room)){setStatus("房間碼必須是 4 位數字。","error");return}
setStatus("正在向 Supabase Edge Function 取得 LiveKit token...");
const res=await fetch(window.P104_CONFIG.TOKEN_FUNCTION_URL,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({room,identity,role:currentRole})});
...
await livekitRoom.connect(window.P104_CONFIG.LIVEKIT_URL,data.token);connectedToolsEl.classList.remove("hidden");showShareQr(room,currentRole);
